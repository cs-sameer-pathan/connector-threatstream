...
query_actions = ["get_import_job_status", "filter_language_query", "delete_incident", "get_incident",
                 "advance_query", "get_import_jobs", "list_threat_model_entity",
                 "list_observables_associated_threat_bulletin", "get_submit_url_status", "get_submitted_url_report",
                 "intelligence_enrichments"]

tb_action = ["list_threat_bulletins", "submit_urls_files", "list_incidents_by_indicator", "fetch_all_incidents"]
...
def get_all_record(resp_json, params, config):
    if resp_json["meta"]["total_count"] != 0:
        ...
        return {'message': 'Executed successfully returned no data',
...
def make_rest_call(endpoint, config, result):
    ...
    try:
        response = request("GET", endpoint_url, params=generate_payload(config, None), verify=config.get('verify_ssl'))
        if response.status_code == 200:
            ...
            logger.error("Failure: make_rest_call: Status: {0} {1}".format(str(response.status_code),
                                                                           str(response.text)))
            raise ConnectorError("Status: {0} {1}".format(str(response.status_code), str(response.text)))
...
def add_attachment_to_tb(tb_id, reference_id, config):
    try:
        ...
        response = request("POST", endpoint_file, params=payload, files=files, verify=config.get("verify_ssl"))
        if response.status_code == 201:
            return response.json()
        else:
            logger.error("Attachment Creation Failed {0}: {1}".format(response.status_code, response.reason))
            raise ConnectorError("Attachment Creation Failed {0}: {1}".format(response.status_code, response.reason))
...
def import_observables(config, params):
    ...
    try:
        ...
        classification = classification_dict.get(params.get('classification'), 'private')
        ...
        expiration_ts = handle_date(expiration_day.get(params.get('expiration_ts'), '90 days'))
        ...
        if source_confidence_weight:
            files.setdefault('source_confidence_weight', (None, source_confidence_weight))
        ...
        if reference_id:
            file_path, file_name = from_cyops_download_file(reference_id)
            files.setdefault('file', (file_name, open(file_path, 'rb')))

        endpoint = server_url + IMPORT_OBSERVABLES

        response = request("POST", endpoint, params=payload, files=files, verify=config.get('verify_ssl'))
...
def create_incident(config, params):
    try:
        ...
        response = request("POST", endpoint, headers=header, params=payload, data=json.dumps(query_data),
                           verify=config.get('verify_ssl'))
        if response.status_code == 201:
            return response.json()
        else:
            logger.error("Failure {0}: {1}".format(response.status_code, response.reason))
...
def update_incident(config, params):
    try:
        ...
        header = {'Content-Type': 'application/json'}
        result.pop('value')
        response = request("PATCH", endpoint, headers=header, params=payload, data=json.dumps(result),
                           verify=config.get('verify_ssl'))
        if response.status_code == 202:
            return response.json()
        else:
...
def list_incidents(config, params):
    ...
    try:
        ...
        if params.get('value', None):
            ...
            operation_details['endpoint'] = "/api/v1/incident/associated_with_intelligence/?value={value}"
...
def api_request(config, params, operation_details):
    try:
        ...
        if operation_details['operation'] in query_actions:
            ...
            endpoint = server_url + operation_details['endpoint'].format(value=param_value)
            ...
            payload = generate_payload_filter(config, params, itype_dict.get(operation_details['operation']))
        ...
        response = request(operation_details['http_method'], endpoint, params=payload, verify=config.get('verify_ssl'))

        if response.status_code == 200:
            if operation_details['operation'] in list(set(resp_list) | set(query_actions)):
                resp_json = response.json()
                if params.get('record_number') == "Fetch All Records":
            ...
            return {"result": "Successfully deleted the incident with ID {0}".format(params.get('value'))}

        raise ConnectorError("{0}:{1} {2}".format(response.status_code, response.reason,
...
def check_health(config):
    try:
        ...
        if response:
            ...
            raise ConnectorError('Failed to connect to Anomali ThreatStream'
...
def get_status(config, params):
    try:
        ...
        for i in range(len(status_list)):
            response_dict[status_list[i].get('display_name')] = status_list[i].get('id') \
                if build_dict \
...
def list_threat_bulletins(config, params):
    try:
        ...
        if params["query"]:
            operation_details["endpoint"] = "/api/v1/tipreport/?{0}".format(params.get("query"))
            params.pop("query")
        else:
...
def list_threat_model_entity(config, params):
    try:
        ...
        operation_details["endpoint"] = "/api/v1/tipreport/{0}/{1}".format(params.get("id"),
                                                                           params.get("entity_type").lower())
        resp = api_request(config, params, operation_details)
        return resp
    except Exception as err:
        logger.error("Failure {0}".format(str(err)))
...
def intelligence_enrichments(config, params):
    try:
        ...
        if services == "Passive DNS":
            ...
            operation_details["endpoint"] = "/api/v1/pdns/{0}/".format(itype.lower()) + "{value}/"

        elif services == "Recorded Future":
            operation_details["http_method"] = "GET"
            operation_details["operation"] = "intelligence_enrichments"
            operation_details["endpoint"] = "/api/v1/recorded_future/search/{0}/".format(itype.lower()) + "{value}/"
